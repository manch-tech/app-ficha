// ============================================================
// shared.js - Manoel Pedra - Original + Supabase
// ============================================================
...
const DOCS_PADRAO = [
  {
    id: "termo-gps-padrao",
    titulo: "Termo de Declaracao de Conflito - GPS",
    url: "./ficha/termo.html",
    descricao: "Baixe, preencha, assine e envie via WhatsApp",
    obrigatorio: true
  }
];
...
function gerarProtocolo(){
  ...
  return "MP-"+d.getFullYear()+pad(d.getMonth()+1)+pad(d.getDate())
    +"-"+pad(d.getHours())+pad(d.getMinutes())+pad(d.getSeconds())+rnd;
}
...
async function sbUpsert(table,rows){
  ...
  try{ await sb.from(table).upsert(rows,{onConflict:"id"}); }
  catch(e){ console.warn("upsert erro",table,e); }
}
async function sbDelete(table,id){
  const sb=getSupabase();
  if(!sb) return;
  try{ await sb.from(table).delete().eq("id",id); }
  catch(e){ console.warn("delete erro",e); }
}
async function sbFetchAll(table){
  ...
  try{
    ...
    console.warn("fetch erro",table,e);
    return null;
  }
}
...
const Store={
  ...
  setCodigos:(v)=>{
    ...
    const rows=v.map(c=>({
      ...
      codigo:c.codigo,
      payload:c,
      created_at:c.data||new Date().toISOString()
    }));
  ...
  setFichas:(v)=>{
    ...
    const rows=v.map(f=>({
      ...
      payload:f,
      created_at:f.dataISO||new Date().toISOString()
    }));
    sbUpsert("fichas",rows);
  },
  getWhats:()=>{
    try{ return localStorage.getItem("manoel_whats_v2")||WHATSAPP_PADRAO; }
    catch{ return WHATSAPP_PADRAO; }
  },
  setWhats:(v)=>{ try{localStorage.setItem("manoel_whats_v2",v);}catch{} },
  getSenha:()=>{
    try{ return localStorage.getItem("manoel_admin_senha")||SENHA_PADRAO; }
    catch{ return SENHA_PADRAO; }
  },
  ...
  syncFromSupabase:async()=>{
    ...
    if(codigos&&codigos.length){
      const lista=codigos.map(r=>r.payload||{
    ...
    if(docs&&docs.length){
      const lista=docs.map(r=>r.payload||{
        id:r.id,
        titulo:r.titulo,
        url:r.url,
...
function Icon(o){
  return React.createElement("svg",{
    ...
    className:o.className||"h-4 w-4"
  },React.createElement("path",{d:o.path}));
}
const ICONS={
